...
describe('Power', function () {
    ...
    it('raises a negative number to a fractional exponent', function (done) {
        request.get('/arithmetic?operation=power&operand1=-8&operand2=1/3')
            .expect(400)
            .end(function (err, res) {
                expect(res.body).to.eql({ error: "Invalid operand2: 1/3" });
                done();
            });
    });
});
